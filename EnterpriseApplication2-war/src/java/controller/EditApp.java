...
public class EditApp extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        Animal allUsers = userService.getAnimalById(id);

        // Set the list of users as a request attribute
        request.setAttribute("animal", allUsers);
        request.setAttribute("d", date);
        request.setAttribute("apid", appId);

        // Forward the request to the JSP file for rendering
        request.getRequestDispatcher("editApp.jsp?apid="+appId).forward(request, response);
    }
